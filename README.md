...
전투 v2 를 붙이며 새로 드러난 것 둘:

- **§6 공성 규칙**(포위·성문·원군)이 아직 없다. 수비군이 성 안에 서게 자리는
  잡았지만 성벽은 아직 그냥 못 지나가는 땅이다 — 공성전이 성벽을 낀 야전으로
  판정된다.
- **성 수비대의 계열이 성에 있는 인물을 따른다.** 「계열은 장수를 따른다」(§1.2)를
  그대로 적용한 결과인데, 문관·책사만 남은 성은 12,000 을 두고도 기병 6,000에게
  깨진다. 주둔군은 그 인물의 사병이 아니므로 규칙을 나눠야 할 자리로 보인다.

**76 거점 판의 밸런스 재조정**도 그대로 남아 있다 (고구려 30.0 / 백제 0.0 /
신라 70.0, 결착률 0%).
